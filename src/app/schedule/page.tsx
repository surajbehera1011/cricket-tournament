...
export default function SchedulePage() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-slate-800">Match Schedule & Fixtures</h1>
      ...
      <div className="flex justify-center mb-8">
        <div className="inline-flex gap-1 bg-white p-1.5 rounded-2xl shadow-lg border border-brand-100/50">
          <button
            ...
            className={`px-6 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${sport === "cricket" ? "bg-brand-600 text-white shadow-md" : "text-slate-500 hover:bg-brand-50"}`}
          >
            🏏 Cricket
          </button>
          <button
            onClick={() => setSport("pickleball")}
            className={`px-6 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${sport === "pickleball" ? "bg-emerald-600 text-white shadow-md" : "text-slate-500 hover:bg-emerald-50"}`}
          >
            🏓 Pickleball
          </button>
        </div>
        ...
        <div>
          {readyTeams.length < 2 ? (
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-6 text-center">
              ...
              <h2 className="text-xl font-bold text-amber-800 mb-2">Cricket Schedule Not Available Yet</h2>
                ...
                {Object.entries(rounds).map(([round, matches]) => (
                  <div key={round}>
                    ...
                    <div className="grid gap-3 sm:grid-cols-2">
                      {matches.map((match) => (
                        <div key={match.id} className="bg-white rounded-2xl border border-brand-100/50 p-4 hover:shadow-md transition-all">
                          <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3">Match #{match.matchNumber}</div>
                          <div className="flex items-center justify-between">
                            ...
                            <div className="mx-3 px-3 py-1 rounded-lg bg-brand-50 text-brand-600 text-xs font-extrabold flex-shrink-0">VS</div>
      ...
      {sport === "pickleball" && (
        <div className="space-y-10">
          {pbRegs.length === 0 ? (
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-6 text-center">
              <span className="text-4xl mb-3 block">🏓</span>
              <h2 className="text-xl font-bold text-amber-800 mb-2">Pickleball Fixtures Not Available Yet</h2>
              <p className="text-sm text-amber-600 max-w-md mx-auto">
            ...
            PB_CATEGORIES.map((cat) => {
                ...
                return (
                  <div key={cat.key}>
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <span className="text-sm text-slate-400 font-medium">({entries.length} entry - need at least 2)</span>
                    </div>
              ...
              return (
                <div key={cat.key}>
                  ...
                  <p className="text-xs text-slate-400 mb-4 ml-4">{entries.length} {isSingles ? "players" : "pairs"} &middot; Round-robin format</p>
                  <div className="grid gap-3 sm:grid-cols-2">
                    {fixtures.map((f) => (
                      <div key={f.id} className={`bg-white rounded-2xl border ${cat.border} p-4 hover:shadow-md transition-all`}>
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2 flex-1 min-w-0">
                            ...
                            <p className="text-sm font-bold text-slate-800 truncate">{entryName(f.entry1)}</p>
                          </div>
                          <div className={`mx-2 px-2.5 py-1 rounded-lg ${cat.bg} ${cat.color} text-xs font-extrabold flex-shrink-0`}>
                            VS
                          </div>
                          <div className="flex items-center gap-2 flex-1 justify-end min-w-0">
                            <p className="text-sm font-bold text-slate-800 truncate text-right">{entryName(f.entry2)}</p>
                            <div className={`w-8 h-8 rounded-full ${cat.bg} ${cat.color} flex items-center justify-center text-xs font-bold flex-shrink-0`}>
